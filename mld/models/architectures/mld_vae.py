...
class MldVae(nn.Module):
    ...
    def __init__(self,
        ...
        self.skel_embedding_s1 = nn.Linear(input_feats, self.latent_dim)
        self.skel_embedding_s2 = nn.Linear(33, self.latent_dim)
        self.skel_embedding_s3 = nn.Linear(18, self.latent_dim)
        self.skel_embedding_s4 = nn.Linear(3, self.latent_dim)
        self.final_layer_s1 = nn.Linear(self.latent_dim, output_feats)
        self.final_layer_s2 = nn.Linear(self.latent_dim, 33)
        self.final_layer_s3 = nn.Linear(self.latent_dim, 18)
        self.final_layer_s4 = nn.Linear(self.latent_dim, 3)
    ...
    def encode(
        ...
        if nfeats == 263 or nfeats == 251:
            x = self.skel_embedding_s1(x)
        elif nfeats == 33:
            x = self.skel_embedding_s2(x)
        elif nfeats == 18:
            x = self.skel_embedding_s3(x)
        elif nfeats == 3:
            x = self.skel_embedding_s4(x)
    ...
    def decode(self, z: Tensor, lengths: List[int], scale: str):
        ...
        if self.arch == "all_encoder":
            ...
            if self.pe_type == "actor":
                ...
                # ).squeeze(0)
        if scale == 's1':
            output = self.final_layer_s1(output)
        elif scale == 's2':
            output = self.final_layer_s2(output)
        elif scale == 's3':
            output = self.final_layer_s3(output)
        elif scale == 's4':
            output = self.final_layer_s4(output)
