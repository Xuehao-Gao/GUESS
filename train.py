...
def main():
    ...
    if cfg.TRAIN.PRETRAINED_VAE:
        ...
        vae_dict_s1 = OrderedDict()
        for k, v in state_dict.items():
            if k.split(".")[0] == "vae_s1":
                name = k.replace("vae_s1.", "")
                vae_dict_s1[name] = v
        model.vae_s1.load_state_dict(vae_dict_s1, strict=True)
        vae_dict_s2 = OrderedDict()
        for k, v in state_dict.items():
            if k.split(".")[0] == "vae_s2":
                name = k.replace("vae_s2.", "")
                vae_dict_s2[name] = v
        model.vae_s2.load_state_dict(vae_dict_s2, strict=True)
        vae_dict_s3 = OrderedDict()
        for k, v in state_dict.items():
            if k.split(".")[0] == "vae_s3":
                name = k.replace("vae_s3.", "")
                vae_dict_s3[name] = v
        model.vae_s3.load_state_dict(vae_dict_s3, strict=True)
